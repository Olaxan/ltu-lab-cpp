...
namespace efiilj {
	...
	public:
		...
		template <typename T> static bool getNum(T &out, std::string prompt = "> ", int min = INT_MIN, int max = INT_MAX);
		template <typename T> static bool getNum(T &out, char exit, std::string prompt = "> ", int min = INT_MIN, int max = INT_MAX);
